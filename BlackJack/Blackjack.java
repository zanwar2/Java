...
public class Blackjack extends ConsoleProgram
{
    ...
    private String getPlayerMove()
    {
        while(true)
        {
            String move = readLine("Enter move (hit/stand/dd): ");
            move = move.toLowerCase();
            
            if(move.equals("hit") || move.equals("stand") || move.equals("dd"))
            {
                return move;
            }
     ...
     */
    private boolean playerTurn(Hand player, Deck deck, int bet)
    {
        while(true)
        {
            ...
            /**if(move.equals("insurance"))
            {
                if(dealer.get(dealer.size-1).equals(ACE))
                {
    ...
    private boolean doubleDown(int bet,Deck deck, Hand player)
    {
        bet = bet*2;
        Card c = deck.deal();
        System.out.println("Your card was: " + c);
    ...
    private double playRound(double bankroll)
    {
        ...
        boolean playerBusted = playerTurn(player, deck, bet);
        if(dd.equals("yes"))
        {
            bet = bet*2;
        }
